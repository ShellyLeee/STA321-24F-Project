...
public class FinalMapper3 extends Mapper<LongWritable, Text, LongWritable, Text> {

    private static final long TIME_WINDOW_MILLIS = 600000; // 时间窗口：毫秒
    ...
    public static long calculateTimeWindowID(long tradetime) {
        ...
        // 获取输入时间的毫秒数
        long currentTimeInMillis = getTimeInMillis(tradetime);
        ...
        if (currentTimeInMillis >= morningStartInMillis && currentTimeInMillis < morningEndInMillis) {
            // 早上 9:30 - 11:30 的时间段，计算属于哪个窗口
            timeWindowID = (currentTimeInMillis - morningStartInMillis) / TIME_WINDOW_MILLIS + 1;
        } else if (currentTimeInMillis >= afternoonStartInMillis && currentTimeInMillis < afternoonEndInMillis) {
            long interval = (morningEndInMillis - morningStartInMillis) / TIME_WINDOW_MILLIS;
            // 下午 13:00 - 15:00 的时间段，计算属于哪个窗口
            timeWindowID = (currentTimeInMillis - afternoonStartInMillis) / TIME_WINDOW_MILLIS + 1 + interval;
        } else if (currentTimeInMillis == afternoonEndInMillis) {
            long interval = (morningEndInMillis - morningStartInMillis) / TIME_WINDOW_MILLIS;
            timeWindowID = (currentTimeInMillis - afternoonStartInMillis) / TIME_WINDOW_MILLIS + (morningEndInMillis - morningStartInMillis) / TIME_WINDOW_MILLIS;
        }
