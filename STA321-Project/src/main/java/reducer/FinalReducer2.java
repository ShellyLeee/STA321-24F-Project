...
public class FinalReducer2 extends Reducer<LongWritable, Text, Text, Text> {

    private static final double CIRCULATION_STOCK = 17170245800.0; // 流通盘总量

    private static final long TIME_WINDOW_SECOND = 600; // 时间窗口
    ...
    // 时间秒数的加法运算
    public static String addTimeInSeconds(String time, long n, long b) {
        // 解析输入的时间，获取小时、分钟和秒
        ...
        // 计算总的秒数
        long totalSeconds = hour * 3600 + minute * 60 + second + (n * b);

        // 计算新的小时、分钟和秒
        long newHour = (totalSeconds / 3600) % 24;  // 处理24小时制
        long newMinute = (totalSeconds / 60) % 60;
    ...
    public static String calculateTimeInterval(long timeWindowID) {
        ...
        long interval = (morningEndInSeconds - morningStartInSeconds) / TIME_WINDOW_SECOND;

        String morningStart = String.valueOf(MORNING_START).substring(8, 14); // 提取 "HHmmss"
        String afternoonStart = String.valueOf(AFTERNOON_START).substring(8, 14); // 提取 "HHmmss"

        if (timeWindowID <= interval) {
            // 早上 9:30 - 11:30 的时间段，计算属于哪个区间
            timeWindowBegin = addTimeInSeconds(morningStart, timeWindowID - 1, TIME_WINDOW_SECOND);
            timeWindowEnd = addTimeInSeconds(morningStart, timeWindowID, TIME_WINDOW_SECOND);
        } else {
            // 下午 13:00 - 15:00 的时间段，计算属于哪个区间
            timeWindowBegin = addTimeInSeconds(afternoonStart, timeWindowID - 1 - interval, TIME_WINDOW_SECOND);
            timeWindowEnd = addTimeInSeconds(afternoonStart, timeWindowID - interval, TIME_WINDOW_SECOND);
        }

        return "20190102" + timeWindowBegin + "000 to 20190102" + timeWindowEnd + "000";
    }
