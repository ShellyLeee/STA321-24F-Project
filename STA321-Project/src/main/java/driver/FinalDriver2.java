...
import reducer.FinalReducer2;


public class FinalDriver2 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // 设置 Mapper
        MultipleInputs.addInputPath(job, new Path(TRADE_INPUT_PATH1), TextInputFormat.class, FinalMapper2.class);
        MultipleInputs.addInputPath(job, new Path(TRADE_INPUT_PATH2), TextInputFormat.class, FinalMapper2.class);
        job.setMapOutputKeyClass(LongWritable.class);  // Mapper的输出Key类型
        job.setMapOutputValueClass(Text.class);        // Mapper的输出Value类型

        // 设置 Reducer
        job.setReducerClass(FinalReducer2.class);
